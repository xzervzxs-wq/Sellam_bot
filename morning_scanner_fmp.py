...
def get_fmp_data(symbol, target_date):
    """جلب شموع 5 دقائق من FMP Stable API"""
    try:
...
# =========================================================
# 4. جلب شموع اليوم وحفظها في CSV
# =========================================================
def save_daily_candles_to_csv(tickers, target_date):
    """جلب شموع اليوم من 9:30 إلى 10:00 وحفظها في ملف CSV"""
    all_candles = []
    
    print(f"\n📊 جاري جلب الشموع من 9:30 AM إلى 10:00 AM...")
    
    for i, symbol in enumerate(tickers, 1):
        try:
            print(f"⏳ [{i}/{len(tickers)}] جلب شموع {symbol}...", end='\r')
            
            df = get_fmp_data(symbol, target_date)
            if df.empty:
                continue
            
            # فلتر الشموع من 9:30 إلى 10:00
            mask = (df.index.time >= time(9, 30)) & (df.index.time <= time(10, 0))
            morning_candles = df[mask]
            
            if morning_candles.empty:
                continue
            
            # إضافة السهم والتاريخ والوقت
            ...
            time_module.sleep(0.1)  # تأخير صغير لتجنب التثنيل
    ...
    if all_candles:
        ...
        filename = f"morning_candles_9_30_10_00_{timestamp}.csv"
        
        df_result.to_csv(filename, index=False, encoding='utf-8')
        
        print(f"\n\n✅ تم حفظ {len(all_candles)} شمعة من {len(df_result['symbol'].unique())} سهم في: {filename}")
        print(f"📊 ملخص الملف:")
        print(f"   - عدد الأسهم: {len(df_result['symbol'].unique())}")
        print(f"   - إجمالي الشموع: {len(all_candles)}")
        print(f"   - متوسط الشموع لكل سهم: {len(all_candles) // len(df_result['symbol'].unique())}")
        
        return filename
    else:
        print(f"\n⚠️ لم يتم جلب أي شموع")
        return None
...
def main():
    ...
    print("="*50)
    print(f"🛡️ MORNING SCANNER (FMP Stable API)")
    print("="*50)
    ...
    # جلب قائمة الأسهم (300 سهم)
    print("\n🔍 جاري جلب 300 سهم من Finviz Elite...")
    ...
    # حفظ الشموع في CSV
    csv_file = save_daily_candles_to_csv(tickers, now_ny)
    ...
    # ملخص الملفات المحفوظة
    if csv_file:
        print(f"\n✅ تم حفظ شموع اليوم في: {csv_file}")
