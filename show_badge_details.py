...
print(f"\n📊 تفاصيل الأداء للأيام المحددة ({len(TARGET_DAYS)} أيام):")
print("-" * 75)
print(f"{'Badge':<15} | {'Total':<6} | {'Wins':<6} | {'Losses':<6} | {'Win Rate':<8} | {'Net PL':<8}")
print("-" * 75)
...
for badge in ['👑 Elite', '🥇 Gold', '🥈 Silver']:
    ...
    losses = len(subset[subset['outcome'].isin(['loss', 'neutral'])])
    win_rate = (wins / total * 100) if total > 0 else 0
    net_pl = subset['pl'].sum()
    
    print(f"{badge:<15} | {total:<6} | {wins:<6} | {losses:<6} | {win_rate:<7.1f}% | {net_pl:<8.1f}")
