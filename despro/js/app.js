...
function selectPlat(plat) {
    ...
    document.querySelectorAll('.platform-btn').forEach(btn => {
        btn.classList.remove('active', 'border-sky-600', 'bg-sky-50', 'text-sky-600', 'shadow-md', 'scale-105');
         // Add back default classes if needed, or rely on base class
    });
...
function addSocialHandleToCanvas() {
    ...
    const chosenColor = document.getElementById('handleColor').value;
    const chosenFont = document.getElementById('fontSelect').value;
    const card = document.getElementById('card'); // منطقة العمل الرئيسية
    
    // إنشاء الحاوية للعنصر
    const wrapper = document.createElement('div');
    wrapper.className = 'absolute inline-flex items-center gap-2 px-2 py-1 cursor-grab active:cursor-grabbing hover:outline hover:outline-2 hover:outline-blue-500 rounded-lg group';
    
    // تحديد موقع عشوائي قليلاً في الوسط
    const cardRect = card.getBoundingClientRect();
    const top = (card.offsetHeight / 2) - 20;
    const left = (card.offsetWidth / 2) - 60;
    ...
    wrapper.innerHTML = `
        <div class="pointer-events-none flex items-center gap-2 select-none" dir="ltr" style="display: flex; align-items: center; gap: 8px;">
             <div style="font-size: 24px; color: ${chosenColor}; display: flex; align-items: center; justify-content: center;">
                ${iconHtml}
            </div>
            <span style="font-size: 18px; font-weight: 900; color: ${chosenColor}; line-height: 1; font-family: ${chosenFont}; white-space: nowrap;">
                ${handle}
            </span>
        </div>
        <!-- زر الحذف يظهر عند التحويم -->
        <button onclick="removeElement(this.parentElement)" class="absolute -top-3 -right-3 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity shadow-sm z-50">
    ...
    // تفعيل التفاعل (السحب، التكبير، الدوران)
    // نستخدم 'box' لأننا نريدها تتصرف كمجموعة واحدة
    setupInteract(wrapper, 'box');
...
// دالة مساعدة لحذف العنصر (إذا لم تكن موجودة)
