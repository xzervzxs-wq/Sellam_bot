...
function mobileShowSheet(type) {
    ...
    switch(type) {
        case 'text':
            ...
            sheetContent = `
                <div class="space-y-3">
                    <textarea id="mobile-text-input" rows="3" placeholder="اكتب النص هنا..." 
                        class="w-full p-3 bg-white border border-[#cbd5e1] rounded-xl text-sm outline-none focus:border-[#6366f1] text-[#1e293b] resize-none"></textarea>
                    <button onclick="mobileAddText()" class="w-full bg-[#6366f1] text-white py-3 rounded-xl font-bold">
        ...
        case 'image':
            ...
            sheetContent = `
                <div class="grid grid-cols-2 gap-3">
                    <label class="flex flex-col items-center justify-center p-6 bg-[#f8fafc] border-2 border-dashed border-[#6366f1] rounded-xl cursor-pointer">
                        <i class="fas fa-image text-3xl text-[#6366f1] mb-2"></i>
                        <span class="text-sm font-bold text-[#1e293b]">صورة عادية</span>
                        <input type="file" onchange="addRegularImage(this); mobileCloseSheet();" class="hidden" accept="image/*">
                    </label>
                    <label class="flex flex-col items-center justify-center p-6 bg-[#f8fafc] border-2 border-dashed border-[#10b981] rounded-xl cursor-pointer">
                        <i class="fas fa-fill-drip text-3xl text-[#10b981] mb-2"></i>
                        <span class="text-sm font-bold text-[#1e293b]">خلفية</span>
                        <input type="file" onchange="addBackground(this); mobileCloseSheet();" class="hidden" accept="image/*">
                    </label>
                </div>
            `;
            break;
            ...
            sheetContent = `
                <div class="space-y-3">
                    <div class="grid grid-cols-4 gap-2">
                        <button onclick="openAssetsPanel('templates'); mobileCloseSheet();" class="flex flex-col items-center p-3 bg-[#f8fafc] rounded-xl">
                            <i class="fas fa-palette text-xl text-[#6366f1] mb-1"></i>
                            <span class="text-[10px] font-bold">قوالب</span>
                        </button>
                        <button onclick="openAssetsPanel('stickers'); mobileCloseSheet();" class="flex flex-col items-center p-3 bg-[#f8fafc] rounded-xl">
                            <i class="fas fa-star text-xl text-[#f59e0b] mb-1"></i>
                            <span class="text-[10px] font-bold">ملصقات</span>
                        </button>
                        <button onclick="openAssetsPanel('frames'); mobileCloseSheet();" class="flex flex-col items-center p-3 bg-[#f8fafc] rounded-xl">
                            <i class="far fa-square text-xl text-[#8b5cf6] mb-1"></i>
                            <span class="text-[10px] font-bold">إطارات</span>
                        </button>
                        <button onclick="openAssetsPanel('backgrounds'); mobileCloseSheet();" class="flex flex-col items-center p-3 bg-[#f8fafc] rounded-xl">
                            <i class="fas fa-image text-xl text-[#10b981] mb-1"></i>
                            <span class="text-[10px] font-bold">خلفيات</span>
                        </button>
                    </div>
                    <button onclick="addShape(); mobileCloseSheet();" class="w-full flex items-center justify-center gap-2 p-3 bg-[#6366f1] text-white rounded-xl font-bold">
                        <i class="fas fa-shapes"></i> إضافة شكل
                    </button>
                </div>
            `;
            break;
        ...
        case 'layers':
            sheetTitle = 'الطبقات';
            const layers = document.querySelectorAll('#card .draggable-el');
            let layersHtml = '<div class="space-y-2 max-h-[40vh] overflow-y-auto">';
            if (layers.length === 0) {
                layersHtml += '<p class="text-center text-[#64748b] py-4">لا توجد طبقات</p>';
            } else {
                layers.forEach((layer, index) => {
                    const isSelected = layer.classList.contains('selected');
                    const isLocked = layer.classList.contains('is-locked');
                    const type = layer.classList.contains('text-layer') ? 'نص' : 
                                 layer.classList.contains('image-layer') ? 'صورة' : 'إطار';
                    const icon = layer.classList.contains('text-layer') ? 'fa-font' : 
                                 layer.classList.contains('image-layer') ? 'fa-image' : 'fa-square';
                    layersHtml += `
                        <div class="flex items-center gap-2 p-2 rounded-lg ${isSelected ? 'bg-[#6366f1] text-white' : 'bg-[#f8fafc]'}" 
                             onclick="selectLayerByIndex(${index}); mobileCloseSheet();">
                            <i class="fas ${icon}"></i>
                            <span class="flex-1 text-sm font-bold">${type} ${index + 1}</span>
                            ${isLocked ? '<i class="fas fa-lock text-xs"></i>' : ''}
                        </div>
                    `;
                });
            }
            layersHtml += '</div>';
            sheetContent = layersHtml;
            break;
            
        case 'export':
            sheetTitle = 'تصدير وحفظ';
            sheetContent = `
                <div class="space-y-3">
                    <button onclick="saveAsImage(); mobileCloseSheet();" class="w-full flex items-center justify-center gap-2 p-4 bg-[#6366f1] text-white rounded-xl font-bold">
                        <i class="fas fa-download"></i> حفظ كصورة
                    </button>
                    <button onclick="printDesign(); mobileCloseSheet();" class="w-full flex items-center justify-center gap-2 p-4 bg-[#10b981] text-white rounded-xl font-bold">
                        <i class="fas fa-print"></i> طباعة AirPrint
                    </button>
                    <button onclick="saveProject(); mobileCloseSheet();" class="w-full flex items-center justify-center gap-2 p-4 bg-[#f59e0b] text-white rounded-xl font-bold">
                        <i class="fas fa-save"></i> حفظ المشروع
                    </button>
                    <label class="w-full flex items-center justify-center gap-2 p-4 bg-[#8b5cf6] text-white rounded-xl font-bold cursor-pointer">
                        <i class="fas fa-folder-open"></i> فتح مشروع
                        <input type="file" onchange="loadProject(this); mobileCloseSheet();" class="hidden" accept=".json">
                    </label>
                </div>
            `;
            break;
        ...
        case 'more':
            ...
            sheetContent = `
                <div class="space-y-3">
                    <button onclick="createNewProject(); mobileCloseSheet();" class="w-full flex items-center gap-3 p-3 bg-[#f8fafc] rounded-xl">
                        <i class="fas fa-file-circle-plus text-[#6366f1]"></i>
                        <span class="font-bold">مشروع جديد</span>
                    </button>
                    <button onclick="toggleDarkMode(); mobileCloseSheet();" class="w-full flex items-center gap-3 p-3 bg-[#f8fafc] rounded-xl">
                        <i class="fas fa-moon text-[#6366f1]"></i>
                        <span class="font-bold">الوضع الليلي</span>
                    </button>
                    <button onclick="undo(); mobileCloseSheet();" class="w-full flex items-center gap-3 p-3 bg-[#f8fafc] rounded-xl">
                        <i class="fas fa-undo text-[#6366f1]"></i>
                        <span class="font-bold">تراجع</span>
                    </button>
                    <button onclick="redo(); mobileCloseSheet();" class="w-full flex items-center gap-3 p-3 bg-[#f8fafc] rounded-xl">
                        <i class="fas fa-redo text-[#6366f1]"></i>
                        <span class="font-bold">إعادة</span>
                    </button>
                    <button onclick="openAssetsPanel('templates'); mobileCloseSheet();" class="w-full flex items-center gap-3 p-3 bg-[#f8fafc] rounded-xl">
                        <i class="fas fa-palette text-[#6366f1]"></i>
                        <span class="font-bold">القوالب</span>
                    </button>
                </div>
            `;
            break;
    ...
    content.innerHTML = sheetContent;
...
function mobileAddText() {
    const input = document.getElementById('mobile-text-input');
    if (input && input.value.trim()) {
        document.getElementById('user-text-input').value = input.value;
        addUserText();
        mobileCloseSheet();
    }
}
...
function selectLayerByIndex(index) {
    const layers = document.querySelectorAll('#card .draggable-el');
    if (layers[index]) {
        selectEl(layers[index]);
    }
}

// إغلاق الـ Sheet عند الضغط على Escape
