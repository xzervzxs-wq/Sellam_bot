...
print("="*70)
print("📊 EODHD FILTERED SCREENER (0.02-10$ + 200K VOLUME)")
print("="*70)

# =========================================================
# 1. تصفية الأسهم من FMP حسب السعر والسيولة
# =========================================================
print("\n🔍 الخطوة 1: تصفية الأسهم من FMP...")
print("   المعايير:")
print("   - السعر: 0.02 - 10 دولار")
print("   - السيولة: 200,000 على الأقل\n")

try:
    # جلب البيانات من Finviz Elite
    ...
    print(f"✅ تم العثور على {len(df_filtered)} سهم يطابق المعايير\n")
    
    if len(df_filtered) > 0:
        print("📋 قائمة الأسهم:")
        print("-" * 70)
        for idx, row in df_filtered.head(20).iterrows():
            print(f"  {row['Ticker']:<8} | ${row['Price']:<7.2f} | {int(row['Volume']):>12,}")
        if len(df_filtered) > 20:
            print(f"  ... و {len(df_filtered) - 20} سهم آخر")
        print()
    
    filtered_stocks = df_filtered['Ticker'].tolist()

except Exception as e:
    print(f"❌ خطأ في تصفية الأسهم: {str(e)}")
    filtered_stocks = []
...
if all_candles:
    ...
    filename = f"eodhd_candles_filtered_0_02_10_200k_{timestamp}.csv"
    ...
    print(f"\n📄 الأسهم المضمنة:")
    stocks_with_data = df_result['symbol'].unique()
    for symbol in sorted(stocks_with_data):
        count = len(df_result[df_result['symbol'] == symbol])
        print(f"   • {symbol}: {count} شمعة")
