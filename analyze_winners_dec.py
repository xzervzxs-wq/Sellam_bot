...
print(f"\n📊 تحليل الفائزين vs الخاسرين (Score Analysis):")
print("-" * 80)
print(f"{'Badge':<15} | {'Outcome':<8} | {'Count':<5} | {'Avg Score':<10} | {'Avg Body':<10}")
print("-" * 80)

for badge in ['👑 Elite', '🥇 Gold', '🥈 Silver']:
    for outcome in ['win', 'loss']: # Group neutral with loss for simplicity or ignore
        subset = df_res[(df_res['badge'] == badge) & (df_res['outcome'] == outcome)]
        if len(subset) == 0: continue
        
        avg_score = subset['score'].mean()
        avg_body = subset['avg_body'].mean()
        print(f"{badge:<15} | {outcome:<8} | {len(subset):<5} | {avg_score:<10.2f} | {avg_body:<10.2f}")

print("-" * 80)

print("\n🏆 Top Winning Patterns in Gold Badge:")
gold_wins = df_res[(df_res['badge'] == '🥇 Gold') & (df_res['outcome'] == 'win')]
print(gold_wins['pattern'].value_counts().head())

print("\n💀 Top Losing Patterns in Gold Badge:")
gold_losses = df_res[(df_res['badge'] == '🥇 Gold') & (df_res['outcome'] != 'win')]
print(gold_losses['pattern'].value_counts().head())

print("\n🔍 Score Distribution for Gold Losers:")
print(gold_losses['score'].describe())
