// ==========================================
        ...
        function addAssetToCanvas(src, colorable, categoryName) {
            ...
            img.onload = function() {
                ...
                wrapper.style.left = Math.max(10, (cardW - w) / 2) + 'px';
                wrapper.style.top = Math.max(10, (cardH - h) / 2) + 'px';
        ...
        window.initSmartEraserCanvas = function() {
            ...
            function end(e) {
                ...
                if(points.length < 10) {
                    ...
                    ctx.clearRect(0, 0, smartEraserCanvas.width, smartEraserCanvas.height);
                    points = [];
                    showSmartToolGuide('الممحاة الذكية');
                    return;
                }
        ...
        function initSmartFillCanvas() {
            ...
            function endDraw(e) {
                ...
                if(points.length < 10) {
                    ...
                    showSmartToolGuide('التلوين الذكي');
                    return;
                }
    ...
    if (sessionStr) {
        try {
            ...
            if (expiryDate >= today) {
                ...
                if(subtitleEl) {
                    subtitleEl.innerHTML = '<span>مساحة مخصصة</span><span style="color: #cccccc; margin: 0 8px; font-weight: 300;">|</span><span>حيث أنت ✨</span>';
                ...
                setTimeout(function() {
                    ...
                    if(el) {
                        el.innerHTML = '<span>مساحة مخصصة</span><span style="color: #cccccc; margin: 0 8px; font-weight: 300;">|</span><span>حيث أنت ✨</span>';
    ...
    elementsArray.forEach((element, index) => {
        ...
        layerItem.innerHTML = '<div class="flex-1 flex items-center gap-2 min-w-0">' +
            ...
            '<div class="flex-1 min-w-0">' +
                '<div class="text-[10px] font-bold truncate" style="' + ((displayName === 'Background' || displayName === 'خلفية') ? 'color: #a855f7; font-weight: 800;' : '') + '">' + displayName + '</div>' +
            '</div>' +
        '</div>' +
...
// --- دالة عرض دليل استخدام الأدوات الذكية ---
function showSmartToolGuide(toolName) {
    const modalId = 'smart-tool-guide-modal';
    ...
    div.innerHTML = `
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-scale-up border border-indigo-100 flex flex-col">
            <div class="bg-gradient-to-r from-indigo-600 to-violet-600 p-6 text-center relative overflow-hidden shrink-0">
                ...
                <div class="relative z-10">
                    <div class="w-16 h-16 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center mx-auto mb-4 border border-white/30 shadow-inner">
                        <i class="fas fa-hand-pointer text-3xl text-white animate-pulse"></i>
                    </div>
                    <h3 class="text-xl font-bold text-white mb-1">كيفية استخدام ${toolName}</h3>
                    <p class="text-indigo-100 text-sm">مجرد نقرة لا تكفي!</p>
                </div>
            </div>
            <div class="p-6 overflow-y-auto max-h-[60vh]">
                <div class="bg-indigo-50 rounded-xl p-4 mb-6 border border-indigo-100 flex items-start gap-3">
                    <div class="bg-white p-2 rounded-lg shadow-sm text-indigo-600 shrink-0">
                        <i class="fas fa-draw-polygon text-xl"></i>
                    </div>
                    <div>
                        <h4 class="font-bold text-gray-800 text-sm mb-1">الرسم المستمر مطلوب</h4>
                        <p class="text-gray-600 text-xs leading-relaxed">
                            هذه أداة يدوية تتطلب منك <span class="font-bold text-indigo-600">الضغط باستمرار والرسم</span> حول المنطقة التي تريد تحديدها. النقرة الواحدة لن تعمل.
                        </p>
                    </div>
                </div>
                ...
                <div class="space-y-4 mb-2">
                    <div class="flex items-center gap-3 text-sm text-gray-600">
                        <span class="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center font-bold text-xs text-indigo-600 shrink-0">1</span>
                        <span>اضغط بإصبعك (أو الماوس) على بداية المنطقة</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm text-gray-600">
                        <span class="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center font-bold text-xs text-indigo-600 shrink-0">2</span>
                        <span><span class="font-bold text-red-500">لا ترفع إصبعك!</span> استمر بالضغط والتحريك</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm text-gray-600">
                        <span class="w-6 h-6 rounded-full bg-gray-100 flex items-center justify-center font-bold text-xs text-indigo-600 shrink-0">3</span>
                        <span>أغلق الشكل بالعودة لنقطة البداية ثم ارفع إصبعك</span>
                    </div>
                </div>
            </div>
            <div class="p-6 pt-0 mt-auto">
                <button onclick="document.getElementById('${modalId}').remove()" class="w-full py-3 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white rounded-xl font-bold transition shadow-lg shadow-indigo-200">
                    فهمت، سأحاول الرسم الآن! 🎨
                </button>
            </div>
        </div>
    `;
