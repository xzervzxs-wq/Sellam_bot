#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
اختبار نهائي لخوارزمية Strict Rhythm
"""
import pandas as pd
import time
import sys

start_time = time.time()

print("\n" + "="*70)
print("🧬 اختبار خوارزمية Strict Rhythm (النسخة النهائية)")
print("="*70)

# 1. تحميل البيانات
print("\n📂 المرحلة 1: تحميل البيانات...")
try:
    df_test = pd.read_csv('test_candles_20251223_170248.csv')
    df_patterns = pd.read_csv('successful_patterns_clean.csv')
    
    print(f"  ✅ بيانات الاختبار: {len(df_test)} صف")
    print(f"  ✅ بيانات الأنماط: {len(df_patterns)} صف")
    print(f"  ✅ أسهم فريدة: {df_test['symbol'].nunique()} في الاختبار، {df_patterns['symbol'].nunique()} في الأنماط")
except Exception as e:
    print(f"❌ خطأ: {e}")
    sys.exit(1)

# 2. دوال المعالجة
def extract_dna(candles):
    """استخراج الحمض النووي للشموع"""
    dna = []
    if isinstance(candles, pd.DataFrame):
        candles = candles[['open', 'high', 'low', 'close']].values
    
    for c in candles:
        try:
            o, h, l, cl = float(c[0]), float(c[1]), float(c[2]), float(c[3])
            tr = h - l
            if tr == 0: tr = 0.0001
            
            dna.append({
                'body_r': abs(cl - o) / tr,
                'upper_r': (h - max(o, cl)) / tr,
                'lower_r': (min(o, cl) - l) / tr,
                'dir': 1 if cl >= o else -1,
                'size': (abs(cl - o) / o * 100) if o > 0 else 0
            })
        except Exception as e:
            print(f"  ⚠️ خطأ في معالجة الشمعة: {e}")
            continue
    
    return dna

def calculate_similarity(curr_dna, pattern_dna):
    """حساب التشابه باستخدام Strict Rhythm"""
    if len(curr_dna) != len(pattern_dna):
        return 0
    
    score = 100.0
    for curr, pattern in zip(curr_dna, pattern_dna):
        # 1. مقارنة الاتجاه
        if curr['dir'] != pattern['dir']:
            score -= 50
        
        # 2. مقارنة حجم الجسم
        body_diff = abs(curr['body_r'] - pattern['body_r'])
        if body_diff > 0.15:
            score -= body_diff * 30
        
        # 3. مقارنة حجم الشمعة الكلي
        size_diff = abs(curr['size'] - pattern['size'])
        if size_diff > 0.4:
            score -= (size_diff - 0.4) * 20
    
    return max(0, score)

# 3. تحميل الأنماط والمقارنة
print("\n📊 المرحلة 2: استخراج الأنماط والمقارنة...")

# إنشاء قاموس الأنماط
patterns_dict = {}
try:
    for symbol, group in df_patterns.groupby('symbol'):
        if len(group) >= 6:
            candles = group.iloc[:6][['open', 'high', 'low', 'close']].values
            patterns_dict[symbol] = extract_dna(candles)
except Exception as e:
    print(f"❌ خطأ في استخراج الأنماط: {e}")
    sys.exit(1)

print(f"  ✅ عدد الأنماط المستخرجة: {len(patterns_dict)}")

# المقارنة
results = []
all_scores = []
stocks = df_test['symbol'].unique()
processed = 0

print(f"\n  🔍 جاري فحص {len(stocks)} سهم...")

for idx, stock in enumerate(stocks):
    try:
        stock_data = df_test[df_test['symbol'] == stock].sort_values('time')
        
        if len(stock_data) < 6:
            continue
        
        candles = stock_data.head(6)[['open', 'high', 'low', 'close']].values
        curr_dna = extract_dna(candles)
        
        if not curr_dna or len(curr_dna) != 6:
            continue
        
        # المقارنة مع جميع الأنماط
        scores = {}
        for pname, pdna in patterns_dict.items():
            scores[pname] = calculate_similarity(curr_dna, pdna)
        
        if scores:
            avg_score = sum(scores.values()) / len(scores)
            max_score = max(scores.values())
            best_pattern = max(scores, key=scores.get)
            
            all_scores.append(avg_score)
            results.append({
                'symbol': stock,
                'avg_match': avg_score,
                'best_match': max_score,
                'best_pattern': best_pattern
            })
            processed += 1
    
    except Exception as e:
        print(f"    ⚠️ خطأ مع {stock}: {e}")
        continue

print(f"  ✅ تم معالجة {processed} سهم بنجاح")

# الإحصائيات
print("\n" + "="*70)
print("📈 النتائج والإحصائيات:")
print("="*70)

if all_scores:
    avg_all = sum(all_scores) / len(all_scores)
    print(f"  📊 متوسط المطابقة: {avg_all:.2f}%")
    print(f"  🔝 أعلى مطابقة: {max(all_scores):.2f}%")
    print(f"  🔻 أقل مطابقة: {min(all_scores):.2f}%")
    
    excellent = sum(1 for s in all_scores if s >= 90)
    good = sum(1 for s in all_scores if 70 <= s < 90)
    okay = sum(1 for s in all_scores if 50 <= s < 70)
    poor = sum(1 for s in all_scores if s < 50)
    
    print(f"\n  📊 توزيع النتائج:")
    print(f"    ⭐⭐⭐⭐⭐ ممتاز (90%+):      {excellent:>3} ({100*excellent/len(all_scores):>5.1f}%)")
    print(f"    ⭐⭐⭐⭐   جيد (70-90%):    {good:>3} ({100*good/len(all_scores):>5.1f}%)")
    print(f"    ⭐⭐⭐     حسن (50-70%):    {okay:>3} ({100*okay/len(all_scores):>5.1f}%)")
    print(f"    ⭐       ضعيف (<50%):     {poor:>3} ({100*poor/len(all_scores):>5.1f}%)")

# أفضل الأسهم
if results:
    print("\n" + "="*70)
    print("🏆 أفضل 25 سهم مطابقة:")
    print("="*70)
    
    df_results = pd.DataFrame(results)
    df_sorted = df_results.sort_values('avg_match', ascending=False)
    
    print(f"\n{'#':<3} {'Symbol':<10} {'Avg %':<10} {'Best %':<10} {'Pattern':<15}")
    print("-" * 70)
    
    for idx, (_, row) in enumerate(df_sorted.head(25).iterrows(), 1):
        print(f"{idx:<3} {row['symbol']:<10} {row['avg_match']:>8.1f}% {row['best_match']:>8.1f}% {row['best_pattern']:<15}")
    
    # حفظ النتائج
    output_file = 'matching_results_final.csv'
    df_sorted.to_csv(output_file, index=False)
    print(f"\n✅ تم حفظ جميع النتائج في: {output_file}")

elapsed = time.time() - start_time
print(f"\n⏱️  الوقت المستغرق: {elapsed:.2f} ثانية")
print("\n✅ اكتمل الاختبار بنجاح!")
